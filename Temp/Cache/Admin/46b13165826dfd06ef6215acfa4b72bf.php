        ...
        <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                  ...
                  <td align="center"><?php echo ($vo["money"]); ?></td>
					...
					<a href="<?php echo U(GROUP_NAME.'/User/inviteInfo',array('user' => $vo['phone']));?>">查看推荐</a> |
                    	<?php if($vo['status'] == 1){ ?>
                        ...
                        <a href="javascript:changepass('<?php echo ($vo["id"]); ?>');">修改密码</a> |
                      	<a href="javascript:changenum('<?php echo ($vo["id"]); ?>');">修改推荐人数</a> |
                        <a href="javascript:del('<?php echo ($vo["phone"]); ?>','<?php echo U(GROUP_NAME.'/User/del',array('id'=>$vo['id'],'user'=>$vo['phone']));?>');">删除</a>
                    </td>
      ...
      function changenum(uid){
		layer.prompt({title: '输入数字，并确认', formType: 0}, function(num, index){
			if(num == '' || num == null){
				...
				$.post(
					"<?php echo U(GROUP_NAME.'/User/changenum');?>",
					{id:uid,num:num},
					function (data,state){
						if(state != "success"){
							layer.msg("网络通讯失败!");
