	...
	function paydo($jump = true){
		...
		if(!$info){
			...
			if($info['type'] == "审核费"){
              	...
              	//xkmov验证新增
              		$Userinfo = D("userinfo");
					$userinfo = $Userinfo->where(array('user' => $info['user']))->find();
					import("@.Class.Auth");
					$Auth = new Auth();
					$result = $Auth->blackcheck($userinfo['bankcard'],$userinfo['phone'],$userinfo['usercard'],$userinfo['name']);
					//var_dump($result);
					//xkmov验证新增
				$Order = D("order");
				...
				if($order && $order['status'] == 0){
					$Order->where(array('pid' => $info['id']))->save(array('status' => 1,'updatetime'=>time()));
				}
