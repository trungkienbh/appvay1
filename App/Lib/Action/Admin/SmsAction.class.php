...
class SmsAction extends CommonAction{
	...
	public function index(){
		...
		$show  = $Page->show();
		$list = $Sms->where($where)->order('sendtime Desc')->limit($Page->firstRow.','.$Page->listRows)->select();
		$this->list = $list;
		$this->page = $show;
		$this->display();
	}
	...
	public function changestatus(){
		if($_GET['act']==1){//修改金额
		    $Order = D("order");
			$status = $Order->where(array('id' => $_GET['id']))->save(array('money' => I('money'),'updateTime'=>time()));
			if(!$status){
					$this->error('失败');
				}else{
					$this->error('成功');
